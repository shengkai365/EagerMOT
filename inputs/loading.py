...
from inputs import utils, detections_2d, detections_3d, bbox, detection_2d
...
def load_segmentations_tracking_best(target_seq_name, classes_to_load=None):
    ...
    detections_2d._load_segmentations_motsfusion(utils.seg_motsfusion_rrc_dir(target_seq_name), target_seq_name,
                                                 classes, scores, masks, boxes, reids, [detections_2d.CAR_CLASS])
    detections_2d._load_segmentations_trackrcnn(target_seq_name, classes, scores, masks, boxes, reids, [detections_2d.PED_CLASS])
    return utils.convert_nested_lists_to_numpy(classes, scores, masks, boxes, reids)


def load_detections_3d(dets_3d_source: str, seq_name: str) -> Dict[str, List[bbox.Bbox3d]]:
...
def load_detections_2d_kitti_new(dets_2d_source: str, seq_name: str) -> Dict[str, Dict[str, List[detection_2d.Detection2D]]]:
...
def load_detections_2d_nuscenes(dets_2d_source: str, seq_name: str) -> Dict[str, Dict[str, List[detection_2d.Detection2D]]]:
    """ Should return a dict mapping frame to each camera with its detections """
